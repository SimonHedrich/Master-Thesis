...
| `automatic_image_qualitiy_filtering.md` | Design notes and rationale for the automated image quality filtering pipeline (`scripts/filter_dataset_quality.py`): staged funnel (metadata → heuristics → MegaDetector → Florence-2 VLM), critique of cloud VLM approaches, and per-source strategy. |
