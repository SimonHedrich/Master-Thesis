...
| `yolov5s_model.py` | Factories: `yolov5s_model` (loads pretrained, swaps detect head for `NUM_CLASSES`), `model_optimizer` (3 param groups: BN / conv / bias), `model_scheduler` (`OneCycleLR`; stepped every batch in the training loop). |
