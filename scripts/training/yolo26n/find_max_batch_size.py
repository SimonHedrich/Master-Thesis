...
Not required to match yolov5s' BATCH_SIZE — VRAM footprint differs by
architecture (docs/2026-04-29_gpu_training_options.md). Set
constants.BATCH_SIZE to the largest power-of-two reported here before a real
(non-smoke) training run.

Run instructions
----------------
From the repository root (dependencies are managed by uv):

    PYTHONPATH=. uv run -m scripts.training.yolo26n.find_max_batch_size
...
Inside Docker (same image used for training):
    ...
    docker run --gpus all --rm \\
        ...
        python -m scripts.training.yolo26n.find_max_batch_size
...
from scripts.training.yolo26n.yolo26n_model import model_optimizer, yolo26n_model
