"""Smoke test for the yolo26n loss adapter and eval-decode path.

This is the risk-mitigation deliverable for
docs/plans/2026-06-30_yolo26-kd-and-teacher-finetune-implementation-plan.md §1.4:
YOLO26n's Detect head (end2end=True) is NMS-free, so evaluation.py's decode
path has no yolov5-equivalent to copy verbatim. Get the decode shape/
coordinate-space wrong and per-class AP would silently look plausible while
being computed on mis-decoded boxes. This test validates the mechanism on
synthetic data (cheap, CPU-only, seconds) before any real training run.
...
Checks:
  ...
  6. evaluation.evaluate() runs end-to-end on a tiny (4-image) DataLoader
     built from CocoYoloDataset(ANNOTATIONS_VAL, augment=False) on an
...
def run_evaluate_plumbing_check() -> bool:
    print("\n6. evaluation.evaluate() end-to-end plumbing (random-init model, 4 val images)")
    try:
        from scripts.training.yolo26n.evaluation import evaluate
    except ImportError as exc:
        return check(False, "scripts.training.yolo26n.evaluation is importable", detail=str(exc))
...
if __name__ == "__main__":
    ...
    try:
        ...
        all_ok &= run_evaluate_plumbing_check()
    except Exception:
        traceback.print_exc()
        all_ok = False
