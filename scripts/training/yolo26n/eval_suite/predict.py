"""Load a trained YOLO26n checkpoint and cache COCO-format predictions JSON.

This module decouples expensive GPU inference from cheap scoring.  It writes a
predictions JSON whose schema is a frozen contract (identical to
scripts.training.yolov5s.eval_suite.predict's); the scoring module reads that
file without ever touching the model.
...
def load_checkpoint(
    ...
    """Build the YOLO26n architecture and load weights from a training checkpoint.
    ...
    * **Raw state_dict** — a ``dict`` of ``{str: Tensor}`` with no ``"model"``
    ...
    # Build fresh architecture (sets model.args, model.names, max_det, etc.)
    model, _ = yolo26n_model(num_classes=num_classes, weights=None, device=device)
...
def run_inference(
    ...
    Output JSON schema (frozen contract — identical to yolov5s.eval_suite.predict)
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    .. code-block:: json
    ...
    Note: yolo26n's Detect head (end2end=True) is NMS-free — conf_thres/
    iou_thres are accepted for call-site parity with yolov5s' predict.py but
    are unused here; only max_det (wired via Detect.max_det) filters detections.
    ...
    logger.info(
        "running inference: %d images, batch=%d, max_det=%d (NMS-free: conf/iou thresholds unused)",
        len(dataset),
        batch_size,
        max_det,
    )
    ...
    with torch.no_grad():
        ...
        for imgs, _targets, paths, shapes in pbar:
            ...
            # yolo26n_model()). model(imgs) in eval mode returns (y, preds); y
            # is (batch, min(max_det, num_anchors), 6) = [x1,y1,x2,y2,score,
            # cls_idx] in the SAME letterboxed-canvas pixel space as yolov5's
            # NMS output, so every line below this point is unchanged from
            # yolov5s/eval_suite/predict.py.
            raw = model(imgs)
