"""Standalone evaluation entrypoint for a trained YOLO26n checkpoint.
...
Thin re-export: reuses grouping/scoring/report from
scripts.training.yolov5s.eval_suite unchanged (confirmed model-agnostic — zero
yolov5-specific imports, pure consumers of the frozen predictions-JSON
contract + COCO annotation JSON) and only swaps in this package's own
(NMS-free) ``predict`` module.

Usage
-----
    # evaluate best.pt from the latest training run (default):
    python -m scripts.training.yolo26n.eval_suite.run_evaluation

    # a specific run, or an explicit checkpoint:
    python -m scripts.training.yolo26n.eval_suite.run_evaluation \
        --run-dir scripts/training/yolo26n/model_exports/yolo26n-20260701-233434
    python -m scripts.training.yolo26n.eval_suite.run_evaluation \
        --checkpoint .../yolo26n-20260701-233434/best.pt

    # smoke test on 40 images per domain, CPU:
    python -m scripts.training.yolo26n.eval_suite.run_evaluation --limit 40 --device cpu
...
from scripts.training.yolo26n.eval_suite import predict
from scripts.training.yolov5s.eval_suite import grouping, report, scoring
...
def main() -> None:
    p = argparse.ArgumentParser(description="Evaluate a YOLO26n checkpoint (strategy doc §9).")
    p.add_argument(
        "--checkpoint",
        type=Path,
        default=None,
