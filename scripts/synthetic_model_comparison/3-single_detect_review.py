#!/usr/bin/env python3
"""
Stage 3 — Single-Detection Triage Review Server (per generator-comparison cell)

Adapted from scripts/synthetic/4-single_detect_review.py for this
experiment's per-cell layout — pass --generator/--prompt-regime to pick which
cell's md_detections.jsonl / single_detect_flags.jsonl to serve.

Reviews synthetic images where MegaDetector found exactly one significant
animal. Click to flag as MULTI (more animals present than MD detected).
Unclicked cards are treated as SINGLE on commit.

Usage:
    uv run python scripts/synthetic_model_comparison/3-single_detect_review.py \\
        --generator gemini-3.1-flash-image-preview --prompt-regime full [--port 8082]
...
_state: dict = {}
app = FastAPI(title="Synthetic Model Comparison — Triage Review")


# ── Data helpers ──────────────────────────────────────────────────────────────

def _load_items(md_detections: Path) -> list[dict]:
    items = []
...
def _load_decisions(flags_file: Path) -> dict[str, str]:
...
async def startup() -> None:
    ...
    print("\n── Synthetic Model Comparison — Triage ──────────────────", flush=True)
    print(f"  Cell: {_state['generator']}/{_state['prompt_regime']}", flush=True)
    print(f"  Loading from {md_detections} ...", end=" ", flush=True)
    all_items = _load_items(md_detections)
    print(f"{len(all_items)} items with n_significant==1", flush=True)

    decided = _load_decisions(flags_file)
...
async def api_batch(n: int = 20) -> JSONResponse:
    ...
    return JSONResponse({
        "class": cls,
        "batch": [
            {"filepath": it["filepath"], "class": it["class"], "detections": it["detections"]}
            for it in batch
        ],
...
async def api_commit(body: CommitBody) -> JSONResponse:
    ...
    with open(_state["flags_file"], "a", encoding="utf-8") as f:
        for fp, dec in body.decisions.items():
...
# Identical UI/UX to scripts/synthetic/4-single_detect_review.py — only the
# cosmetic title/done-text differ (no dynamic path — this server can point at
# any cell).
...
<title>Synthetic Model Comparison — Triage Review</title>
<style>
:root {
  --bg:       #111;
  --surface:  #1c1c1c;
...
<div id="done-screen">
  ...
  <p style="margin-top:8px">Results saved to this cell's <code>single_detect_flags.jsonl</code></p>
</div>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Synthetic Model Comparison — Single-Detection Triage Server")
    ...
    print(f"Starting triage server for {args.generator}/{args.prompt_regime}")
