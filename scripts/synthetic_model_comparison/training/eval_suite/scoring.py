...
def score(
    ...
    # Build + update in chunks rather than materialising per-image tensors for
    # every image up front — at full test-set scale that means our own list
    # and torchmetrics's internal copy of it exist simultaneously at peak.
    sorted_ids = sorted(image_ids)  # sorted for reproducibility
    for chunk_start in range(0, len(sorted_ids), _SCORE_UPDATE_CHUNK_SIZE):
        chunk_ids = sorted_ids[chunk_start : chunk_start + _SCORE_UPDATE_CHUNK_SIZE]
        tm_preds: list[dict] = []
...
if __name__ == "__main__":
    ...
    # ── Build a fake COCO-format annotation file ──────────────────────────────
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # ── score() — coarse remap (all → class 1) ────────────────────────────
        coarse_remap_all = {1: 1, 2: 1, 3: 1}
        result_coarse = score(real_gt, real_preds, remap=coarse_remap_all, class_metrics=True)
        assert result_coarse["n_gt"] == 3
        print(f"\nCoarse score: map={result_coarse['map']:.4f}  map_50={result_coarse['map_50']:.4f}")
        print(f"  per_class (should be single key=1): {result_coarse['map_per_class']}")
        assert set(result_coarse["map_per_class"].keys()) == {1}, \
            f"coarse remap should collapse to class 1, got {set(result_coarse['map_per_class'].keys())}"
