...
Granularity levels used by this experiment's (trimmed) report
---------------------------------------------------------------
``fine``    — identity remap (category_id → itself); per-species AP.
``coarse``  — category_id → group_id; look-alike clusters merged (used only
              to detect within-group confusion, not scored on its own — the
              ``detect`` class-agnostic granularity isn't part of this
              experiment's Axis C ask and was dropped, see eval_suite/report.py).

Usage example
-------------
::

    from scripts.synthetic_model_comparison.training.eval_suite.grouping import (
        load_coarse_remap,
        identity_remap,
        load_group_labels,
        load_class_to_band,
...
def identity_remap(cat_ids: list[int]) -> dict[int, int]:
    ...
    This is a no-op pass-through included for API symmetry with
    :func:`load_coarse_remap` so both granularity levels can be driven by the
    same scoring code path: ``remap = fine | coarse; then apply remap[cat_id]``.
