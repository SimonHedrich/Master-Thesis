#!/usr/bin/env python3
"""
Stage 5 — Multi-animal bbox labeling server

Interactive per-image bbox editor where MD bboxes are pre-loaded as a starting
point. The user can add, remove, or adjust bboxes, then Save & Next or Skip.

Queue sources:
  - md_detections.jsonl       where n_significant >= 2 or == 0  (train, Stage 3)
  - md_detections_test.jsonl  where n_significant >= 2 or == 0  (test,  Stage 3)
  - single_detect_flags.jsonl where decision == "multi"          (Stage 4, both splits)
...
MD_DETECTIONS_TEST = REPO_ROOT / "data" / "synthetic" / "md_detections_test.jsonl"
FLAGS_FILE         = REPO_ROOT / "data" / "synthetic" / "single_detect_flags.jsonl"
LABELS_FILE        = REPO_ROOT / "data" / "synthetic" / "manual_labels.jsonl"

# ── Global state ───────────────────────────────────────────────────────────────

_state: dict = {}
app = FastAPI(title="Bbox Labeling")
...
def _load_queue() -> list[dict]:
    ...
    for det_file in (MD_DETECTIONS, MD_DETECTIONS_TEST):
...
async def startup() -> None:
    ...
    print("\n── Stage 5: Multi-animal bbox labeling ──────────────────────", flush=True)
...
<title>Bbox Labeling — Stage 5</title>
<style>
:root {
  --bg:      #111;
  --surface: #1c1c1c;
...
<div id="header">
  <span id="logo">Stage 5 &mdash; Bbox Labeling</span>
...
<div id="done-overlay" class="overlay">
  ...
  <p>All multi-animal images in the queue have been processed.</p>
  <p>Results saved to <code>data/synthetic/manual_labels.jsonl</code></p>
...
var bboxes      = [];      // [{xc, yc, w, h, source, conf}, ...] YOLO normalised
var selectedIdx = -1;
var imgData     = null;
var img         = new Image();
var mouseState  = null;    // null | {mode, ...}
var busy        = false;
...
function onMouseDown(e) {
  ...
  // Priority 1: handles of selected bbox
  ...
  // Priority 2: click inside any bbox
...
function onMouseMove(e) {
  ...
  if (mouseState.mode === 'draw') {
    ...
    // Enforce minimum 10px in each dimension
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 5 — Multi-animal bbox labeling server")
    ...
    if args.force:
        _state["force"] = True
    print(f"Starting bbox labeling server — open http://<tailscale-ip>:{args.port} in your browser")
