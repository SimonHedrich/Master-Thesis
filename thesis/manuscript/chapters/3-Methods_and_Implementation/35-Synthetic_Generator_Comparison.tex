...
\Cref{fig:zebra-species} shows a real photograph of each of the three look-alike species, since the distinguishing marks listed in the table's \enquote{Contribution} column are easier to judge by eye than to read.
...
\end{subfigure}
\caption{The three look-alike zebra species of \Cref{tab:generator-comparison-classes}, photographed real animals. The markers named in each caption are the diagnostic features a generator must reproduce for the within-group confusion measured in \Cref{sec:results_per_class_structure} to be low.}
