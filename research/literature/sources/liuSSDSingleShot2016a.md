...
1 UNC Chapel Hill 2 Zoox Inc. 3 Google Inc. 4 University of Michigan, Ann-Arbor 1 wliu@cs.unc.edu, 2 drago@zoox.com, 3 { dumitru,szegedy } @google.com, 4 reedscot@umich.edu, 1 { cyfu,aberg } @cs.unc.edu
...
3 For SSD512 model, we add extra conv12 2 for prediction, set s min to 0.15, and 0.07 on conv4 3.
...
|---------------------------|----------|----------|----------|----------|----------|
| more data augmentation? 1 |          |        4 |        4 |        4 |        4 |
| include { 2 , 2 } box?    |        4 |          |        4 |        4 |        4 |
| include { 1 3 , 3 } box?  |        4 |          |          |        4 |        4 |
| use atrous?               |        4 |        4 |        4 |          |        4 |
...
Atrous is faster. As described in Sec. 3, we used the atrous version of a subsampled VGG16, following DeepLab-LargeFOV [17]. If we use the full VGG16, keeping pool5 with 2 × 2 - s 2 and not subsampling parameters from fc6 and fc7, and add conv5 3 for prediction, the result is about the same while the speed is about 20% slower.

Table 3: Effects of using multiple output layers.

| Prediction source layers from:   | Prediction source layers from:   | Prediction source layers from:   | Prediction source layers from:   | Prediction source layers from:   | Prediction source layers from:   | mAP use boundary boxes?   | mAP use boundary boxes?   |   # Boxes |
|----------------------------------|----------------------------------|----------------------------------|----------------------------------|----------------------------------|----------------------------------|---------------------------|---------------------------|-----------|
...
[4 http://host.robots.ox.ac.uk:8080/leaderboard/displaylb.php?cls=mean&amp;challengeid=11&amp;compid=4](http://host.robots.ox.ac.uk:8080/leaderboard/displaylb.php?cls=mean&challengeid=11&compid=4)

5 For SSD512 model, we add extra conv12 2 for prediction, set s min to 0.1, and 0.04 on conv4 3.

Table 5: COCO test-dev2015 detection results.

| Method      | data        | Avg. Precision, IoU:   |   Avg. Precision, IoU: | Avg. Precision, IoU:   | Avg. Precision, Area:   | Avg. Precision, Area:   | Avg. Precision, Area:   | Avg. Recall, #Dets:   | Avg. Recall, #Dets:   | Avg. Recall, #Dets:   | Avg. Recall, Area:   | Avg. Recall, Area:   | Avg. Recall, Area:   |
|-------------|-------------|------------------------|------------------------|------------------------|-------------------------|-------------------------|-------------------------|-----------------------|-----------------------|-----------------------|----------------------|----------------------|----------------------|
...
|         | VOC2007 test   | VOC2007 test   | VOC2012 test   | VOC2012 test    | COCO test-dev2015   | COCO test-dev2015   |   COCO test-dev2015 |
|---------|----------------|----------------|----------------|-----------------|---------------------|---------------------|---------------------|
| Method  | 07+12 0.5      | 07+12+COCO 0.5 | 07++12 0.5     | 07++12+COCO 0.5 | 0.5:0.95            | trainval35k 0.5     |                0.75 |
